...
import { Send, Settings, Trash2, MessageCircle, CheckCircle, AlertCircle, RefreshCw } from 'lucide-react'
import './App.css'
    ...
    try {
      ...
      if (data.success) {
        const aiMessage = {
          ...
          grammarCorrections: data.grammar_corrections || [],
          timestamp: new Date().toLocaleTimeString()
        }
                    ...
                    {message.grammarCorrections && message.grammarCorrections.length > 0 && (
                      <div className="mt-2 pt-2 border-t border-gray-200">
                        ...
                        {message.grammarCorrections.map((correction, index) => (
                          <div key={index} className="text-xs bg-green-50 p-2 rounded mb-1">
                            <div><span className="font-medium">原文:</span> {correction.original}</div>
                            <div><span className="font-medium">修正:</span> {correction.corrected}</div>
                            {correction.explanation && (
                              <div><span className="font-medium">说明:</span> {correction.explanation}</div>
                            )}
                          </div>
                        ))}
                      </div>
          ...
          <CardContent className="p-4">
            ...
            <div className="flex items-center justify-between mt-2 text-sm text-gray-500">
              ...
              <div className="flex items-center space-x-4">
                ...
                <span>{config.model || config.customModel}</span>
